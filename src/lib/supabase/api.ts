...
export function mapTask(row: Record<string, unknown>): Task {
  return {
    ...
    status_changed_by_profile_id: row.status_changed_by_profile_id
      ...
      : null,
    assignee_notified_at: row.assignee_notified_at
      ? String(row.assignee_notified_at)
      : null,
  };
}
...
export function mapProjectContractorExpense(
  ...
  return {
    ...
    repeat_end_month: row.repeat_end_month
      ? String(row.repeat_end_month).slice(0, 10)
      : null,
    created_at: String(row.created_at ?? ""),
...
const SHARE_ORG_COLS: string = "id, name, slug, disabled_at, share_enabled";
const SHARE_ORG_SETTINGS_COLS: string =
...
const SHARE_PERSON_COLS: string =
...
const SHARE_MILESTONE_COLS: string =
...
const SHARE_ASSIGNMENT_COLS: string =
...
const SHARE_HOLIDAY_CAL_COLS: string = "id, organization_id, name, region";
const SHARE_HOLIDAY_DAY_COLS: string =
  "id, organization_id, calendar_id, date, name";
const SHARE_LEAVE_COLS: string =
  "id, organization_id, person_id, date, kind, status, hours_per_day, notes";

/** Runtime field list; typed as "*" to bypass supabase-js select-string parser. */
...
export async function loadOrgBootstrap(
    ...
    if (
      ...
      if (!legacy.error) {
        unread_mentions = (legacy.data ?? [])
          ...
          .filter((r) => r.comment_id && r.person_id && !r.read_at);
      }
    }
  } else {
  ...
  let unread_assigned_tasks: { task_id: string; person_id: string }[] = [];
  if (sessionPersonId) {
    const assignedUnreadsRes = await supabase
      .from("task_assigned_unreads")
      .select("task_id, person_id")
      .eq("organization_id", orgId)
      .eq("person_id", sessionPersonId);
    if (assignedUnreadsRes.error) {
      if (
        !/relation .*task_assigned_unreads.* does not exist/i.test(
          assignedUnreadsRes.error.message,
        ) &&
        assignedUnreadsRes.error.code !== "42P01"
      ) {
        throw assignedUnreadsRes.error;
      }
      console.warn(
        "task_assigned_unreads missing — apply supabase/migrations/105_task_assigned_notify.sql",
      );
    } else {
      unread_assigned_tasks = (assignedUnreadsRes.data ?? [])
        .map((row) => ({
          task_id: String((row as { task_id: unknown }).task_id),
          person_id: String((row as { person_id: unknown }).person_id),
        }))
...
/** Comments (with extras) for unread task-thread inbox hydration. */
export async function loadCommentsForTaskIds(
  supabase: SupabaseClient,
  orgId: string,
  taskIds: string[],
): Promise<{ tasks: Task[]; task_comments: TaskComment[] }> {
  const ids = [...new Set(taskIds.filter(Boolean))];
  if (ids.length === 0) {
    return { tasks: [], task_comments: [] };
  }
...
export async function loadProjectData(
  ...
  const milestoneCols: string = slim ? SHARE_MILESTONE_COLS : "*";
  const taskListCols: string = slim ? SHARE_TASK_LIST_COLS : "*";
  const taskCols: string = slim ? SHARE_TASK_COLS : "*";
  const assetCols: string = slim ? SHARE_ASSET_COLS : "*";
  const assignmentCols: string = slim ? SHARE_ASSIGNMENT_COLS : "*";
  const [
    milestonesRes,
    taskListsRes,
    tasksRes,
    ...
    supabase
      .from("milestones")
      .select(cols(milestoneCols))
      .eq("organization_id", orgId)
      .eq("project_id", projectId),
    supabase
      .from("task_lists")
      .select(cols(taskListCols))
      .eq("organization_id", orgId)
      .eq("project_id", projectId),
    supabase
      .from("tasks")
      .select(cols(taskCols))
      .eq("organization_id", orgId)
      .eq("project_id", projectId),
    supabase
      .from("project_assets")
      .select(cols(assetCols))
      .eq("organization_id", orgId)
      .eq("project_id", projectId),
    supabase
      .from("assignments")
      .select(cols(assignmentCols))
      .eq("organization_id", orgId)
      .eq("project_id", projectId),
  ]);
...
export async function loadPublicOrgShareData(
  ...
  const assignmentsQuery = supabase
    .from("assignments")
    .select(cols(SHARE_ASSIGNMENT_COLS))
    .eq("organization_id", orgId)
    .or(
      [
        `and(recurrence.eq.none,start_date.lte.${rangeEnd},end_date.gte.${rangeStart})`,
  ...
  const [
    ...
    supabase
      .from("milestones")
      .select(cols(SHARE_MILESTONE_COLS))
      .eq("organization_id", orgId),
    supabase
      .from("task_lists")
      .select(cols(SHARE_TASK_LIST_COLS))
      .eq("organization_id", orgId),
    supabase
      .from("project_assets")
      .select(cols(SHARE_ASSET_COLS))
      .eq("organization_id", orgId),
    supabase.from("tasks").select(cols(SHARE_TASK_COLS)).eq("organization_id", orgId),
    assignmentsQuery,
    supabase
      .from("leave_days")
      .select(cols(SHARE_LEAVE_COLS))
      .eq("organization_id", orgId)
      .gte("date", rangeStart)
      .lte("date", rangeEnd),
  ]);
...
export async function loadShareOrgShell(
  ...
  let membersQuery = supabase
    ...
    .select(cols(SHARE_MEMBER_COLS))
    .eq("organization_id", orgId);
  let expensesQuery = supabase
    .from("project_contractor_expenses")
    .select(cols(SHARE_EXPENSE_COLS))
    .eq("organization_id", orgId);
  if (projectId) {
  ...
  const [
    ...
    supabase.from("organizations").select(cols(SHARE_ORG_COLS)).eq("id", orgId).single(),
    supabase
      .from("organization_settings")
      .select(cols(SHARE_ORG_SETTINGS_COLS))
      .eq("organization_id", orgId)
      .maybeSingle(),
    supabase.from("clients").select(cols(SHARE_CLIENT_COLS)).eq("organization_id", orgId),
    supabase
      .from("projects")
      .select(cols(SHARE_PROJECT_COLS))
      .eq("organization_id", orgId),
    supabase
      .from("people")
      .select(cols(SHARE_PERSON_COLS))
      .eq("organization_id", orgId)
      .is("deleted_at", null),
    membersQuery,
    expensesQuery,
    supabase
      ...
      .select(cols(SHARE_HOLIDAY_CAL_COLS))
      .eq("organization_id", orgId),
    supabase
      .from("holiday_calendar_days")
      .select(cols(SHARE_HOLIDAY_DAY_COLS))
      .eq("organization_id", orgId),
    includeBulletins
      ? supabase
          .from("bulletins")
          .select(cols(SHARE_BULLETIN_COLS))
          .eq("organization_id", orgId)
...
export async function upsertProjectContractorExpenseRow(
  ...
  const payload = {
    ...
    repeat_monthly: Boolean(row.repeat_monthly),
    repeat_end_month: row.repeat_end_month
      ? row.repeat_end_month.slice(0, 10)
      : null,
...
export async function upsertTaskRow(supabase: SupabaseClient, task: Task) {
  ...
  if (
    ...
    updateError &&
    /assignee_notified_at/i.test(updateError.message)
  ) {
    const { assignee_notified_at: _n, ...withoutNotified } = payload;
    const retry = await supabase
      .from("tasks")
      .update(withoutNotified)
      .eq("id", task.id)
      .select("id");
    if (!retry.error && retry.data && retry.data.length > 0) {
      console.warn(
  ...
  if (
    error &&
    /assignee_notified_at/i.test(error.message)
  ) {
    const { assignee_notified_at: _n, ...withoutNotified } = payload;
    ({ error } = await supabase.from("tasks").insert(withoutNotified));
    if (!error) {
      console.warn(
        "tasks.assignee_notified_at missing — apply supabase/migrations/105_task_assigned_notify.sql",
      );
...
function missingAssignedUnreadsTable(message: string, code?: string) {
  return (
    /relation .*task_assigned_unreads.* does not exist/i.test(message) ||
    code === "42P01"
  );
}

export async function upsertTaskAssignedUnreadRow(
  supabase: SupabaseClient,
  row: {
    task_id: string;
    person_id: string;
  ...
  const { error } = await supabase.from("task_assigned_unreads").upsert(
    {
      task_id: row.task_id,
      person_id: row.person_id,
      organization_id: row.organization_id,
  ...
  if (!error) return true;
  if (missingAssignedUnreadsTable(error.message, error.code)) {
    console.warn(
      "task_assigned_unreads missing — apply supabase/migrations/105_task_assigned_notify.sql",
    );
    return false;
...
export async function deleteTaskAssignedUnreadRow(
  supabase: SupabaseClient,
  row: {
    task_id: string;
    person_id: string;
...
/** Delete the mention unread row (subtractive inbox). */
export async function markMentionReadRow(
  ...
  return deleteMentionUnreadRow(supabase, row);
}
