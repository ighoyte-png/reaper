-- Deep-link targets for system bulletins (task in-review, milestone approval).
...
create index if not exists bulletins_task_id_idx on public.bulletins (task_id)
  where task_id is not null;

create index if not exists bulletins_milestone_id_idx on public.bulletins (milestone_id)
  where milestone_id is not null;

-- Include task_id on new in-review notifications.
