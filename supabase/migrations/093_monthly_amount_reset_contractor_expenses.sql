-- Monthly amount retainers + per-month contractor expenses for monthly-reset projects.

create table if not exists public.project_contractor_expenses (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  project_id uuid not null references public.projects(id) on delete cascade,
  person_id uuid not null references public.people(id) on delete cascade,
  -- First day of the calendar month this expense belongs to.
  ...
  constraint project_contractor_expenses_month_key_first_of_month
    check (month_key = date_trunc('month', month_key)::date)
);

create index if not exists project_contractor_expenses_project_month_idx
  on public.project_contractor_expenses (project_id, month_key desc);

create index if not exists project_contractor_expenses_org_idx
  on public.project_contractor_expenses (organization_id);

alter table public.project_contractor_expenses enable row level security;

create policy project_contractor_expenses_select
  on public.project_contractor_expenses for select
  using (organization_id = public.current_org_id());

create policy project_contractor_expenses_write
  on public.project_contractor_expenses for all
  using (
...
as $$
  ...
  expense_totals as (
    select
      ...
      coalesce(sum(
        case
          when coalesce(nullif(pe.cost_rate, 0), nullif(pe.bill_rate, 0), 0) > 0
            then e.amount / coalesce(nullif(pe.cost_rate, 0), pe.bill_rate)
          else 0
        end
      ), 0) as expense_hours
